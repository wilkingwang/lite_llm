...
namespace tiktoken
{
    static auto mergeBytePair(const std::string &piece,
                              const anker::unordered_dense::map<std::string, int> &ranks,
    ...
    static auto encodeBytePair(const std::string &piece,
                               const anker::undered_dense::map<std::string, int> &ranks) -> std::vector<int>
    {
        if (piece.size() == 1)
        {
    ...
    public:
        ...
        auto encode_single_piece(const std::string &text) const -> std::vector<int>
        {
            ...
            return byte_pair_encode(text, encoder);
        }
    ...
    private:
        ...
        auto ordinaryNativeEncode(const std::string &text) const -> std::vector<int>
        {
            ...
            while (re2::RE2::FindAndConsume(&input, *regex, &piece))
            {
                ...
                auto tokens = byte_pair_encode(piece, encoder);
